...
public class FlyingDroneScript : MonoBehaviour {
	...
	private void UpdateFreeMovementState()
	{
		...
		try {
			...
			GetComponent<Rigidbody>().AddForce(dfb * transform.forward * forceMultiplier);
			GetComponent<Rigidbody>().AddForce(dud * transform.up      * forceMultiplier * upDownFactor);
			GetComponent<Rigidbody>().AddForce(drl * transform.right   * forceMultiplier * leftRightFactor);
	...
	private void CheckFolloweeMoved()
	{
		if (curMode == DroneMode.Follow)
		{
			...
			// When following, the drone should maintain its altitude, presumably above the target.
